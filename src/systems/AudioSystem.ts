...
export class AudioSystem {
  ...
  /** Тиха степова тема: повільні акорди-подушки по колу + рідкі мотиви-щипки. */
  startMusic(): void {
    const ctx = this.ensureContext();
    if (!ctx || !this.master) return;
    ...
    const motif = [293.66, 349.23, 392.00, 440.00, 523.25];
    const chordDurationMs = 9000;
    let chordIndex = 0;
    ...
    const playChord = (): void => {
      ...
      chord.forEach((f, i) => {
        this.scene.time.delayedCall(i * 60, () => this.padTone(f, chordDurationMs / 1000 + 1, 0.022));
      });
      chordIndex++;
    };

    const playMotifNote = (): void => {
      if (this.muted || Math.random() < 0.4) return;
      const f = motif[Math.floor(Math.random() * motif.length)] * (Math.random() < 0.5 ? 1 : 2);
      this.tone(f, 1.6, 0.018, 'triangle', (Math.random() - 0.5) * 0.6);
    };

    playChord();
    this.scene.time.addEvent({ delay: chordDurationMs, loop: true, callback: playChord });
    this.scene.time.addEvent({ delay: 3200, loop: true, callback: playMotifNote });
  }

  private padTone(freq: number, duration: number, volume: number): void {

...
export function runAI(scene: GameScene, ai: AIState, difficulty: Difficulty): void {
  // кочовий промисел: AI не полює й не варить сіль — крапає пасивно
  const aiEcon = scene.economy.get(SIDE.ai);
  if (aiEcon) {
    aiEcon.food += 1.1;
    aiEcon.salt += 0.25;
  }
  ...
  if (econ.pop + 3 >= econ.popCap && !isBuilding(scene, 'farm')) tryBuild(scene, 'farm', hall, workers);
...
function tryTrain(scene: GameScene, producer: Building, kind: UnitKind): boolean {
  ...
  if (!scene.economy.hasPopRoom(SIDE.ai, def.food)) {
  ...
  if (!scene.economy.canAfford(SIDE.ai, def.cost)) return false;
  scene.economy.spend(SIDE.ai, def.cost);

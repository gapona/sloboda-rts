...
const WEAPON_RIG: Record<UnitKind, WeaponRig> = {
  ...
  kharakternyk: { ox: 9, oy: 0, originX: 0.1, originY: 0.85, baseRot: -0.2 }
};
...
const SHADOW_KEY: Record<UnitKind, string> = {
  ...
  kharakternyk: 'unit_shadow_m'
};

const BODY_HEIGHT: Record<UnitKind, number> = {
  worker: 40, footman: 44, archer: 44, knight: 52, catapult: 48, kharakternyk: 46
};
...
export class Unit implements IEntity {
  ...
  /** Туманець: уповільнення до часу (time.now) і коефіцієнт. */
  slowUntilMs = 0;
  slowFactor = 1;
  /** Здібність героя готова після цього часу (time.now). */
  abilityReadyAt = 0;
  ...
  /** Сюжетне ім'я (напр. отаман) — показується замість стандартної назви юніта в UI. */
  customName: string | null = null;
  private heroBanner: Phaser.GameObjects.Graphics | null = null;
  ...
  /** Виділяє юніта-героя (отамана тощо): золотий відблиск + прапорець над головою. */
  markAsHero(): void {
    if (this.usingArtSheet) {
      const display = UNIT_ART_DISPLAY[this.unitKind];
      this.sprite.setDisplaySize(display.width * 1.12, display.height * 1.12);
    } else {
      this.sprite.setScale(this.sprite.scaleX * 1.12, this.sprite.scaleY * 1.12);
    }
    this.heroBanner = this.sprite.scene.add.graphics();
    this.heroBanner.setDepth(34);
    this.drawHeroBanner();
  }

  private drawHeroBanner(): void {
    if (!this.heroBanner) return;
    const g = this.heroBanner;
    g.clear();
    const x = this.sprite.x;
    const top = this.sprite.y - BODY_HEIGHT[this.unitKind] * 1.05;
    g.lineStyle(1.5, 0x6b4226, 1);
    g.lineBetween(x, top, x, top + 12);
  ...
  die(): void {
    ...
    this.workBusy = false;
    if (this.heroBanner) { this.heroBanner.destroy(); this.heroBanner = null; }
    if (this.usingArtSheet) {
  ...
  update(): void {
    ...
    if (this.sprite.visible !== this.lastVisible) {
      ...
      this.heroBanner?.setVisible(this.lastVisible);
    }

    // Skip rig updates if off-screen — major perf win
    if (!this.lastVisible) return;
    if (this.heroBanner) this.drawHeroBanner();

...
import { Side, BUILDING, BuildingKind, RACE_COLOR, RESOURCE, TILE, UnitKind, UNIT, Race } from '../config';
import {
...
export class Building implements IEntity {
  ...
  /** Тік врожаю поля: повертає кількість їжі, якщо настав час збору, інакше 0. */
  tickFood(dtMs: number): number {
    if (!this.completed || this.buildingKind !== 'field') return 0;
    this.foodAccumMs += dtMs;
    if (this.foodAccumMs >= RESOURCE.fieldFoodIntervalMs) {
      this.foodAccumMs -= RESOURCE.fieldFoodIntervalMs;
      return RESOURCE.fieldFoodAmount;
    }
    return 0;
  }

import type { AnimalKind, BuildingKind, Race, Side, StoryMapId, UnitKind } from '../config';
...
export interface StoryDialogueLine {
  ...
  /** Ключ портрета: ім'я файлу з assets/sloboda/portraits (напр. 'otaman') або повний шлях. */
  portrait?: string;
}
...
export type StoryAtmosphereTone = 'normal' | 'ashen' | 'forbidden' | 'night';
...
export interface StoryLandmark {
  ...
  race?: Race;
  color?: number;
  /** Для kind 'image': ключ текстури (напр. 'sloboda_ruins') та розмір на мапі. */
  textureKey?: string;
  displayWidth?: number;
  displayHeight?: number;
  /** Чи ховати підпис (за замовчуванням підпис показується). */
  hideLabel?: boolean;
}
...
export interface StoryScriptedUnit {
  ...
  x: number;
  y: number;
  /** Множник швидкості руху відносно базової UNIT[kind].speed (напр. повільний конвой). */
  speedMul?: number;
}
...
export type StoryCondition =
  ...
  | { type: 'resourceCount'; side: Side; resource: 'gold' | 'lumber' | 'salt' | 'food'; count: number }
  | {
      type: 'event';
...
export type StoryEvent =
  ...
  | { type: 'grantResources'; side: Side; gold?: number; lumber?: number; salt?: number; food?: number }
  | { type: 'spawnAnimals'; animals: Array<{ kind: AnimalKind; x: number; y: number }> }
...
export type StoryRuntimeEvent =
  ...
  | { type: 'animalKilled'; animalKind: AnimalKind; bySide?: Side };
...
export interface StoryControllerGameApi {
  ...
  grantResources(side: Side, gold?: number, lumber?: number, salt?: number, food?: number): void;
  spawnAnimals(animals: Array<{ kind: AnimalKind; x: number; y: number }>): void;
  getResourceCount(side: Side, resource: 'gold' | 'lumber' | 'salt' | 'food'): number;
  ...
  setClock(label: string, durationMs: number, icon?: string): void;
  clearClock(): void;
  setDayPhase(phase: 'day' | 'night'): void;
...
export interface StoryStartingUnit {
  ...
  x: number;
  y: number;
  /** Якщо задано — юніт реєструється у storyGroups під цим id (для groupCount-умов). */
  groupId?: string;
  /** Множники характеристик відносно базових UNIT[kind] (для героїв/посилених юнітів). */
  hpMul?: number;
  atkMul?: number;
  /** Сюжетне ім'я — показується замість стандартної назви юніта в UI. */
  customName?: string;
}
...
export interface StoryMapDefinition {
  ...
  /** Якщо задано, хутір (townhall) можна закласти лише на цій точній клітинці (лівий верхній кут footprint), напр. на городищі. */
  townhallSite?: { tx: number; ty: number };
  /** Точка збору соли (солеварня) — якщо задано, носії соли йдуть сюди, а не до найближчого хутора. */
  saltCollectionPoint?: { x: number; y: number };
  /** Множники характеристик для скриптованих юнітів side: ai (татари слабші/сильніші за гравця). */
  enemyStatMultiplier?: { hp?: number; atk?: number };
}

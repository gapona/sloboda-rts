...
export class StoryController {
  ...
  townhallSite(): { tx: number; ty: number } | null {
    return this.definition.townhallSite ?? null;
  }
    ...
    for (const event of events) {
      switch (event.type) {
        ...
        case 'commandGroup':
          ...
          break;
        case 'grantResources':
          this.game.grantResources(event.side, event.gold, event.lumber, event.salt, event.food);
          break;
        case 'spawnAnimals':
          this.game.spawnAnimals(event.animals);
          break;
        ...
        case 'damageOrDestroyBuilding':
          ...
          break;
        case 'setClock':
          this.game.setClock(event.label, event.durationMs, event.icon);
          break;
        case 'clearClock':
          this.game.clearClock();
          break;
        case 'setDayPhase':
          this.game.setDayPhase(event.phase);
          break;
        case 'endGame':
  ...
  private conditionMet(condition: StoryCondition, event: StoryRuntimeEvent, timeMs: number): boolean {
    ...
    switch (condition.type) {
      ...
      case 'resourceCount':
        return this.game.getResourceCount(condition.side, condition.resource) >= condition.count;
      case 'event':
...
function eventMatches(condition: Extract<StoryCondition, { type: 'event' }>, event: StoryRuntimeEvent): boolean {
  ...
  if (condition.animalKind && (event.type !== 'animalKilled' || event.animalKind !== condition.animalKind)) return false;

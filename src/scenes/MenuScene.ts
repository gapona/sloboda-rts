...
import { Race, Difficulty, STORY_MODE_DEFAULTS, STORY_MAP_LABEL, type GameLaunchConfig, type StoryMapId } from '../config';
import menuBgUrl from '../menu/menu_backround.png';
...
export class MenuScene extends Phaser.Scene {
  ...
  private bgEl: HTMLImageElement | null = null;
  ...
  private storyHandler = (e: Event): void => {
    const btn = e.currentTarget as HTMLElement;
    this.startStory(btn.dataset.map as StoryMapId);
  };
  ...
  create(): void {
    ...
    this.bgEl = document.getElementById('menu-bg') as HTMLImageElement | null;

    if (this.bgEl) {
      if (!this.bgEl.src) this.bgEl.src = menuBgUrl;
      this.bgEl.classList.add('visible');
    }
  ...
  private teardown(): void {
    if (this.menuEl) {
      ...
      const storyRow = document.getElementById('menu-story-row');
      if (storyRow) storyRow.innerHTML = '';
    }
    if (this.bgEl) {
  ...
  private buildStoryList(): void {
    const row = document.getElementById('menu-story-row');
    if (!row) return;
    row.innerHTML = '';
    for (const [id, label] of Object.entries(STORY_MAP_LABEL)) {
      const btn = document.createElement('button');
      btn.className = 'menu-story-pick';
      btn.type = 'button';
      btn.dataset.map = id;
      btn.textContent = label;
      btn.addEventListener('click', this.storyHandler);
      row.appendChild(btn);
    }
  }

  private startStory(mapId?: StoryMapId): void {
    this.start({ ...STORY_MODE_DEFAULTS, storyMapId: mapId ?? STORY_MODE_DEFAULTS.storyMapId });
  }
  ...
  private applyArtBackground(): void {
    ...
    if (this.bgEl) this.bgEl.classList.remove('visible');
  }

...
export class BootScene extends Phaser.Scene {
  ...
  preload(): void {
    ...
    this.load.image('salt_deposit', artAssetUrl('assets/sloboda/objects/salt.png'));
    // об'єкти Слободи — для story-лендмарків (kind: 'image', textureKey: 'sloboda_<name>')
    const slobodaObjects = [
      'ruins', 'khutir', 'neutral_hut', 'enemy_camp', 'fog_shrine',
      'kharakternyk_hut', 'sawmill', 'saltmine', 'bridge', 'bridge_site',
      'squad_player', 'squad_enemy', 'gold', 'lumber'
    ];
    for (const name of slobodaObjects) {
      this.load.image(`sloboda_${name}`, artAssetUrl(`assets/sloboda/objects/${name}.png`));
    }
  }
  ...
  private createTexturesAndStart(): void {
    ...
    const nextScene = new URLSearchParams(window.location.search).has('editor') ? 'EditorScene' : 'MenuScene';
  ...
  /** Дика звірина: олень, вепр, туша. Стиль — як у решти процедурного арту. */
  private makeWildlife(): void {
    // ── олень (вид збоку, дивиться вправо) ──
    let g = this.add.graphics();
    // ноги
    g.fillStyle(0x6e4f30, 1);
    g.fillRect(10, 20, 3, 9); g.fillRect(16, 21, 3, 8);
    g.fillRect(24, 20, 3, 9); g.fillRect(29, 21, 3, 8);
    // тіло
    g.fillStyle(0x9a7244, 1);
    g.fillRoundedRect(7, 12, 28, 11, 5);
    g.fillStyle(0xa8804f, 1);
    g.fillRoundedRect(9, 12, 24, 6, 4);
    // шия + голова
    g.fillStyle(0x9a7244, 1);
    g.fillRect(32, 7, 5, 9);
    g.fillRoundedRect(33, 3, 8, 6, 2);
    // морда
    g.fillStyle(0x7a5a36, 1); g.fillRect(40, 5, 3, 3);
    // око
    g.fillStyle(0x1c1410, 1); g.fillRect(37, 4, 1, 2);
    // роги
    g.lineStyle(1, 0xd8c9a8, 1);
    g.beginPath(); g.moveTo(35, 3); g.lineTo(32, -2); g.lineTo(29, -4); g.strokePath();
    g.beginPath(); g.moveTo(33, 0); g.lineTo(30, 1); g.strokePath();
    g.beginPath(); g.moveTo(37, 3); g.lineTo(38, -3); g.lineTo(41, -5); g.strokePath();
    // хвостик і світле черево
    g.fillStyle(0xd8c9a8, 1); g.fillRect(6, 13, 3, 4); g.fillRect(10, 20, 22, 2);
    g.generateTexture('animal_deer', 44, 30);
    g.destroy();

    // ── вепр (масивний, темний) ──
    g = this.add.graphics();
    g.fillStyle(0x3a2e24, 1);
    g.fillRect(8, 18, 4, 8); g.fillRect(15, 19, 4, 7);
    g.fillRect(22, 18, 4, 8); g.fillRect(27, 19, 4, 7);
    // тіло-горб
    g.fillStyle(0x4e3c2c, 1);
    g.fillRoundedRect(5, 8, 28, 13, 6);
    g.fillStyle(0x5e4936, 1);
    g.fillRoundedRect(7, 8, 20, 6, 5);
    // щетина гребенем
    g.fillStyle(0x352a1f, 1);
    g.fillRect(9, 6, 14, 3);
    // голова-рило
    g.fillStyle(0x4e3c2c, 1);
    g.fillRoundedRect(30, 10, 9, 9, 3);
    g.fillStyle(0x6a5440, 1); g.fillRect(38, 13, 3, 4);
    // ікла
    g.fillStyle(0xe8e0cc, 1); g.fillRect(37, 17, 2, 3); g.fillRect(40, 16, 1, 3);
    // око
    g.fillStyle(0xc23a2a, 1); g.fillRect(34, 11, 2, 2);
    g.generateTexture('animal_boar', 44, 28);
    g.destroy();

    // ── вовк (сірий, хижий силует) ──
    g = this.add.graphics();
    g.fillStyle(0x4a4a4e, 1);
    g.fillRect(9, 17, 3, 8); g.fillRect(15, 18, 3, 7);
    g.fillRect(22, 17, 3, 8); g.fillRect(27, 18, 3, 7);
    // тіло, нижче й довше за оленя
    g.fillStyle(0x5e5e64, 1);
    g.fillRoundedRect(6, 10, 26, 9, 4);
    g.fillStyle(0x6e6e74, 1);
    g.fillRoundedRect(8, 10, 20, 4, 3);
    // хвіст поліном
    g.fillStyle(0x55555a, 1);
    g.fillTriangle(6, 12, -2, 8, 5, 16);
    // голова з вишкіреною пащею
    g.fillStyle(0x5e5e64, 1);
    g.fillRoundedRect(29, 6, 9, 8, 2);
    g.fillStyle(0x4a4a4e, 1); g.fillRect(36, 10, 5, 3);
    // вуха
    g.fillStyle(0x4a4a4e, 1); g.fillTriangle(30, 6, 32, 1, 34, 6); g.fillTriangle(34, 6, 36, 2, 38, 6);
    // око-жарина
    g.fillStyle(0xe0b03a, 1); g.fillRect(33, 8, 2, 2);
    // ікла
    g.fillStyle(0xe8e0cc, 1); g.fillRect(37, 12, 1, 2); g.fillRect(39, 12, 1, 2);
    g.generateTexture('animal_wolf', 42, 26);
    g.destroy();

    // ── вовче лігво (нора під корчем, кістки) ──
    g = this.add.graphics();
    // пагорб
    g.fillStyle(0x4e4030, 1);
    g.fillEllipse(26, 22, 46, 24);
    g.fillStyle(0x5c4c38, 1);
    g.fillEllipse(26, 19, 40, 18);
    // чорний зів нори
    g.fillStyle(0x14100c, 1);
    g.fillEllipse(26, 24, 24, 13);
    g.fillStyle(0x000000, 1);
    g.fillEllipse(26, 25, 18, 9);
    // корч зверху
    g.fillStyle(0x3a2e20, 1);
    g.fillRect(10, 6, 30, 5);
    g.fillRect(16, 2, 5, 8); g.fillRect(30, 1, 4, 9);
    // кістки біля входу
    g.fillStyle(0xd8d0bc, 1);
    g.fillRect(8, 30, 8, 2); g.fillEllipse(7, 31, 4, 4); g.fillEllipse(17, 31, 4, 4);
    g.fillRect(38, 31, 6, 2);
    g.generateTexture('wolf_den', 52, 36);
    g.destroy();

    // ── туша (лежить на боці) ──
    g = this.add.graphics();
    // тінь-калюжа
    g.fillStyle(0x6e2018, 0.55);
    g.fillEllipse(22, 22, 30, 9);
    // тіло на боці
    g.fillStyle(0x8a6a42, 1);
    g.fillRoundedRect(8, 12, 26, 10, 5);
    g.fillStyle(0x9a774c, 1);
    g.fillRoundedRect(10, 12, 20, 5, 4);
    // задерті ноги
    g.fillStyle(0x6e4f30, 1);
    g.fillRect(14, 6, 2, 7); g.fillRect(19, 5, 2, 8);
    g.fillRect(25, 6, 2, 7);
    // голова закинута
    g.fillStyle(0x8a6a42, 1); g.fillRoundedRect(32, 14, 7, 6, 2);
    g.generateTexture('carcass', 44, 28);
    g.destroy();
  }
  ...
  private makeUnit(kind: UnitKind, race: Race): void {
    switch (kind) {
      ...
      case 'kharakternyk': return this.makeKharakternyk(race);
    }
  }

  /** Процедурний фолбек характерника/баксы (альянс зазвичай має арт-листи). */
  private makeKharakternyk(race: Race): void {
    const key = `unit_kharakternyk_${race}`;
    if (this.textures.exists(key)) return;
    const W = 34, H = 46;
    const g = this.add.graphics();
    const cx = W / 2;
    const coat = race === 'alliance' ? 0x2c3450 : 0x3c2c20;
    const trim = race === 'alliance' ? 0x6a86c8 : 0xc88a3a;
    // довгий темний жупан
    g.fillStyle(coat, 1);
    g.fillRoundedRect(cx - 8, 14, 16, 26, 4);
    g.fillStyle(trim, 1);
    g.fillRect(cx - 1, 16, 2, 22);
    // голова з оселедцем/каптуром
    g.fillStyle(0xd2a87c, 1);
    g.fillCircle(cx, 9, 6);
    g.fillStyle(0x1c1410, 1);
    g.fillRect(cx - 6, 3, 12, 4);
    if (race === 'alliance') { g.fillRect(cx + 3, 1, 6, 2); }
    // два клинки навхрест за спиною
    g.lineStyle(2, 0xc8d4e8, 1);
    g.beginPath(); g.moveTo(cx - 12, 12); g.lineTo(cx + 12, 34); g.strokePath();
    g.beginPath(); g.moveTo(cx + 12, 12); g.lineTo(cx - 12, 34); g.strokePath();
    // іскра-«характерство»
    g.fillStyle(0x9adcff, 0.9);
    g.fillCircle(cx, 24, 2);
  ...
  private makeBuilding(kind: BuildingKind, race: Race, stage: 'final' | 'stage1' | 'stage2'): void {
    ...
    switch (kind) {
      ...
      case 'field': return this.makeField(race, stage);
    }
  }
  ...
  private buildingSize(kind: BuildingKind): number {
    return (kind === 'farm' || kind === 'tower' || kind === 'field') ? TILE * 2 : TILE * 3;
  }
  ...
  private makeField(race: Race, stage: 'final' | 'stage1' | 'stage2'): void {
    const size = this.buildingSize('field');
    const g = this.add.graphics();
    const key = this.buildingKey('field', race, stage);
    ...
    const soil = 0x6a4a30;
    const soilDark = 0x4e3520;
    const crop = race === 'alliance' ? 0xd6c24a : 0xc89a3c;
    const cropDark = race === 'alliance' ? 0x9a8a2a : 0x8a6a24;

    // Shadow
    g.fillStyle(0x000000, 0.5);
    g.fillEllipse(size / 2, size - 4, size * 0.86, 10);

    // Plowed soil base
    g.fillStyle(soil, 1);
    g.fillRect(4, 6, size - 8, size - 12);
    // Furrow rows
    g.lineStyle(2, soilDark, 0.85);
    for (let y = 11; y < size - 6; y += 7) {
      g.lineBetween(5, y, size - 5, y);
    }

    // Crop sprouts along the furrows
    for (let y = 9; y < size - 6; y += 7) {
      for (let x = 8; x < size - 6; x += 9) {
        g.fillStyle(cropDark, 1);
    ...
    // Wattle fence around the plot
    g.lineStyle(2, WOOD_DARK, 1);
    g.strokeRect(2, 2, size - 4, size - 4);
    g.lineStyle(1, WOOD, 0.8);
    for (let x = 4; x < size - 2; x += 8) {
      g.lineBetween(x, 1, x, 5);
      g.lineBetween(x, size - 5, x, size - 1);
    }

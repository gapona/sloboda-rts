...
export const UNIT_ART_DISPLAY: Record<UnitKind, DisplaySize> = {
  ...
  kharakternyk: { width: 50, height: 50 }
};
...
export const UNIT_SHEET_ASSETS: UnitSheetAsset[] = RACES.flatMap((race) =>
  ...
  // Характерник: арт є лише для alliance (hero-юніт, у horde немає відповідника)
  UNIT_ANIM_STATES
    ...
    .map((anim) => {
      ...
      return {
        type: 'spritesheet' as const,
        group: 'unit' as const,
        key: unitSheetKey('kharakternyk', 'alliance', anim),
        path: `assets/art/units/alliance/kharakternyk_${anim}.png`,
        frameWidth: UNIT_ART_FRAME.width,
        frameHeight: UNIT_ART_FRAME.height,
        race: 'alliance' as const,
        unitKind: 'kharakternyk' as const,
        anim,
        framesPerFacing: meta.frames,
        fps: meta.fps,
        repeat: meta.repeat,
        display: UNIT_ART_DISPLAY.kharakternyk,
        origin: { x: 0.5, y: 0.58 }
      };
    })
);
